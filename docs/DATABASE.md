...
- **`published`** — visible publicly. Requires at least one **qualifying** evidence row (defined precisely below). This is deliberately a low, single bar for Slice 1B, not a per-field completeness check ("birth date AND nationality must both be verified") — a finer-grained rule is a reasonable future hardening step, not required to resolve the invariant this section exists to fix.
...
**Qualifying evidence, precisely.** A `fighter_evidence` row qualifies a fighter for publication when all of the following hold:

1. it belongs to that fighter (`fighter_id` matches);
2. `verification_status = 'verified'`;
3. `confidence IN ('high', 'medium')` — a `'low'`-confidence claim, even if marked `verified`, isn't enough on its own to publish; and
4. its `source_document_id` resolves to an existing `source_documents` row — automatically guaranteed by the `NOT NULL REFERENCES` FK, not a separate runtime check.

One qualifying row is sufficient — it doesn't need to be a specific field (`field_name IS NULL` or any specific column both count). Which fields must individually be verified before publishing is deferred, per ADR-014's revisit triggers.

**Enforcement is service-layer, not a database constraint or trigger — and it lives in the fighters module, not the database layer.** Publishing is fighter-domain business logic (the rule is "what does it mean for a *fighter* to be publishable"), not a generic database concern, so it belongs in `src/modules/fighters/services/publication.ts`, not `src/db/`. The database layer's job is to expose the transaction and query primitives these functions use, not to own the rule itself. Built in Slice 1B — see [SLICE_1_FOUNDATION.md](SLICE_1_FOUNDATION.md).

- **`publishFighter(fighterId)`** — in one transaction: `SELECT ... FROM fighters WHERE id = $1 FOR UPDATE` to lock the fighter row first, then count qualifying evidence rows for that fighter. If zero, roll back and return a typed failure. If at least one, `UPDATE fighters SET publication_status = 'published' WHERE id = $1`, then commit.
- **`deleteFighterEvidence(evidenceId)`** — in one transaction: look up the evidence row's `fighter_id`, then `SELECT ... FROM fighters WHERE id = $fighterId FOR UPDATE` to lock that fighter row **before** deleting anything, delete the evidence row, re-count remaining qualifying evidence for that fighter, and — only if the count is now zero and the fighter's (locked, freshly-read) `publication_status` is `'published'` — set it back to `'draft'` in the same transaction, then commit. **Policy choice: auto-revert, not reject-the-delete.** An admin correcting a mistaken piece of evidence shouldn't be blocked from deleting it; the fighter simply — and visibly, via its own `publication_status` — stops being publicly displayed until new qualifying evidence is added. Rejecting the delete would force admins to publish a replacement fact first or route around the rule entirely, which is worse than quietly unpublishing.

**Why lock the fighter row first, in both functions:** without it, two concurrent operations on the same fighter can interleave unsafely — e.g., `publishFighter` counts one qualifying row and is about to publish, while `deleteFighterEvidence` concurrently deletes that exact row believing the fighter isn't published yet; both commit, and now a `published` fighter has zero qualifying evidence. Acquiring `FOR UPDATE` on the fighter row at the start of each transaction serializes the two operations — whichever starts first holds the lock until it commits, and the second doesn't proceed (or re-reads current state) until the first is done. Evidence rows themselves don't need their own row lock beyond this, since the fighter-row lock is what serializes the two operations that matter.

**Nothing else may write `publication_status`.** Any general-purpose fighter-update function (e.g., a future `updateFighter(fighterId, patch)` used for editing vitals) must exclude `publication_status` from its accepted input type entirely — not by convention, but by the input schema/type simply not having that field, so there's nothing to accidentally pass through. `publishFighter` and the internal revert-to-draft step inside `deleteFighterEvidence` are the only code paths permitted to write that column.

No CHECK constraint or trigger enforces this at the database level in Slice 1B — a direct `UPDATE fighters SET publication_status = 'published'` issued from outside `publishFighter` (bypassing both the module and the type-level restriction above) still isn't stopped by Postgres itself. That's an accepted gap for Slice 1B, consistent with this codebase's convention that all writes go through the service layer (see [ARCHITECTURE.md](ARCHITECTURE.md)'s module boundaries) rather than being reached for directly. **Future hardening, not built now:** a Postgres trigger (`BEFORE UPDATE ... WHEN (NEW.publication_status = 'published' AND OLD.publication_status IS DISTINCT FROM 'published')`, checking for a qualifying evidence row) could turn this into a real database guarantee if service-layer discipline ever proves insufficient — deferred because a hand-rolled trigger is exactly the kind of complexity Slice 1B should avoid introducing without a clean, well-tested implementation already in hand.
